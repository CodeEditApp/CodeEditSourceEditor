...
//    }
    ...
    package var fontCharWidth: CGFloat { (" " as NSString).size(withAttributes: [.font: config.appearance.font]).width }
...
//    }
    ...
    init(
        ...
        self.textView = TextView(
            ...
            lineHeightMultiplier: config.appearance.lineHeight,
            wrapLines: config.appearance.wrapLines,
            isEditable: config.behavior.isEditable,
            isSelectable: config.behavior.isSelectable,
            letterSpacing: config.appearance.letterSpacing,
            useSystemCursor: config.appearance.useSystemCursor,
            delegate: self
        )

        // Initialize guide view
        self.guideView = ReformattingGuideView(
